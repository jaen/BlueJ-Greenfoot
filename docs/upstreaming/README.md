...
|----|-------|--------|--------|
| PR-1: Source model | A | In progress | `common-source-representation` |
...
| Parser selection | Hardcoded `JavaParser` | `@ProjectScoped LanguageRegistry` |
...
`JavaParser` stays as-is. A `@ProjectScoped LanguageRegistry` dispatches to the right parser based on `SourceType`. This avoids invasive refactoring of the 3467-line `JavaParser` and means the Java path has zero regression risk.
...
Introduced as a skeleton in PR-3, methods added organically:
...
A `@ProjectScoped LanguageRegistry` maps `SourceType` -> `LanguageSupport`, so any component needing language-dependent behavior injects `LanguageRegistry` and asks for the right implementation.

## Tracks

| Track | Focus | PRs | Details |
|-------|-------|-----|---------|
| [A: Foundation](tracks/a-foundation/README.md) | Source model + token infrastructure + test corpus | PR-1, PR-2, test corpus | Unified `SourceFile` abstraction, language-agnostic token queries, Kotlin test corpus |
